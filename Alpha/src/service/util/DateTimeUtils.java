...
public class DateTimeUtils {
	 ...
	 * 
	 * @param dateOnly
	 *            a Date instance with date part only
	 * @param timeOnly
	 *            a Date instance with time part only
	...
	public static Date mergeUtcDateTime(Date dateOnly, Date timeOnly) {
		...
		if (timeOnly == null) {
			...
			if ((tc.get(Calendar.YEAR) != epoch.get(Calendar.YEAR))
					...
					|| (tc.get(Calendar.DAY_OF_MONTH) != epoch
							.get(Calendar.DAY_OF_MONTH))) {
				throw new IllegalArgumentException(
						"The supplied time Date is either not constructed in UTC or contains non 0 date values");
			}

			setHms(cal, tc.get(Calendar.HOUR_OF_DAY), tc.get(Calendar.MINUTE),
	...
	public static Date[] splitDateTime(Date dateAndTime) {
		...
		setHms(time, date.get(Calendar.HOUR_OF_DAY), date.get(Calendar.MINUTE),
	...
	/**
	 * This is used to normalize the date to be able to detect a date change.
	 * The hours, minutes, seconds and milliseconds are just set to 0.
	 * 
	 * @param date
	 *            The date which should be normalized.
	...
	public static long daysBetween(long dateOne, long dateTwo) {
		return (normalizeDate(dateOne) - normalizeDate(dateTwo))
				/ IConstants.DAY;
	}

	/**
	 * Makes a minimum of the two dates and returns the smaller of the two
	 * dates. If one of the two dates is null, the other is returned.
	 * 
	 * @param d1
	 *            a date may be null
	 * @param d2
	...
	/**
	 * Makes a maximum of the two dates and returns the greater of the two
	 * dates. If one of the two dates is null, the other is returned.
	 * 
	 * @param d1
	 *            a date may be null
	 ...
	 */
	public static Date createUTCDate(int year, int month, int day, int hour,
			int minute) {
		if (month == 0) {
			throw new InvalidArgumentException(
					"Month cannot be zero, use values 1-12");
		}
		if (day == 0) {
			throw new InvalidArgumentException(
					"Day cannot be zero, use values 1-31");
		}
	 ...
	 */
	public static TimeWindow createTimeWindow(int syear, int smonth, int sday,
			int shour, int sminute, int eyear, int emonth, int eday, int ehour,
			int eminute) {

		return new TimeWindow(
				createUTCDate(syear, smonth, sday, shour, sminute),
				createUTCDate(eyear, emonth, eday, ehour, eminute));
	}
