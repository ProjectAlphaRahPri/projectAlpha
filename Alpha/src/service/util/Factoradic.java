...
public class Factoradic {
	 ...
	 * 
	 * @param decimalNumber
	 *            - the number that has to be converted to factoradic. The
	 *            <tt>decimalNumber < factoradicBase!</tt> (! is factorial)
	 * @param factoradicBase
	 *            - the base of the factoradic number. If there are 'n'
	 *            permuting/permutating elements then the factoradic base is
	 *            also 'n'.
	 * @return int - factoradic number
	 * @throws Exception
	 */
	public static int[] getFactoradic(int decimalNumber, int factoradicBase)
			throws Exception {
		int fact = factorial(factoradicBase) - 1;
		if (decimalNumber > fact) {
			throw new Exception(
					"Improper values: (decimalNumber > factoradicBase!-1) ("
							+ decimalNumber + ">" + fact + ")");
		}
