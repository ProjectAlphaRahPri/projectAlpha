...
public class SystemTime
{
	...
	private static final Date MIN_DB_DATE = DateTimeUtils.createUTCDate(1, 1, 1, 0, 0);

	private static final Date MAX_DB_DATE = DateTimeUtils.createUTCDate(9999, 12, 31, 0, 0);
	 ...
	 * 
	 * @param newTimeMode the time mode that has to be used:
	 ...
	 * 
	 * @param newSystemTime the new system time
	...
	public static boolean setSystemTime(final long newSystemTime)
	{
		if (!isUpdateTimePossible(newSystemTime)) {
			...
			// "Ignored try to set the system backwards from ", new Date(nowTime),
			// " to ",
	...
	{
		// ensure that no seconds an milliseconds are used
		long clearedSystemTime = (newSystemTime / IConstants.MINUTE) * IConstants.MINUTE;
	...
	/**
	 * @param newTimeMode the new system time mode
	 */
