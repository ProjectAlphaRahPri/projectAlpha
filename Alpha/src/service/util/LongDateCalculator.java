...
public class LongDateCalculator
{
	...
	protected static final int TIME_1230 = (12 * IConstants.HOUR) + (30 * IConstants.MINUTE);
	 ...
	 */
	private static final TimeWindow LUNCH_BREAK = new TimeWindow(TIME_1230, TIME_1300);
	...
	static {
		...
		for (int i = 0; i < DST_TIME_PERIODS.length; i++) {
			DST_TIME_PERIODS[i] = DateTimeUtils.getDaylightSavingPeriod(1970 + i, timeZone);
		}
	}
	 ...
	 * 
	 * @param time number that represents date and time in milliseconds
	 * @param tz
	 ...
	 * 
	 * @param start number that represents start date and time in milliseconds
	 * @param end number that represents end date and time (later than start) in
	 *        milliseconds
	 * @return the milliseconds within lunch break
	 */
	public static long getMillisWithinLunchBreak(final long start, final long end)
	{
		...
		return LUNCH_BREAK.getOverlap(LongDateCalculator.getMillisInDay(start), LongDateCalculator
				.getMillisInDay(end));
	}
	 ...
	 * 
	 * @param time number that represents date and time in milliseconds
	 * @param time2 number that represents another date and time in milliseconds
	...
	{
		int millisInDay = (int) ((time + getTimeOffset(time, tz)) % IConstants.DAY);
	...
	public static long getDayStart(final long time)
	{
		...
		if (LongDateCalculator.getMillisInDay(temp) != 0) {
			...
			if (LongDateCalculator.getMillisInDay(temp) < 12 * IConstants.HOUR) {

				// the day is 23h long (summertime change --> lets adapt this
				temp = (temp - 1 * IConstants.HOUR);
			} else if (LongDateCalculator.getMillisInDay(temp) >= 12 * IConstants.HOUR) {
	...
	public static int getWeekday(final long time, final TimeZone tz)
	{
		...
		// thursday has the constant int value 5 : difference between 5 and 1 is 4
		// -> +4 for the weekday
		int dayOfWeek = (int) ((day + 4) % 7);

		// +1 because first day (sunday) starts with constant int value 1 instead
		// of 0
	 ...
	 * 
	 * @param time for which the offset is needed
	 * @param tz timezone in which the date is created or null if it's UTC date
	...
	public static long getTimeOffset(final long time, final TimeZone tz)
	{
		...
		if (year >= 0 && year < DST_TIME_PERIODS.length) {
			...
			dstPeriod = DateTimeUtils.getDaylightSavingPeriod(1970 + year, DEFAULT_TIME_ZONE);
		}
	 ...
	 * 
	 * @param startTime time in milliseconds representing the begin of the
	 *        interval
	 * @param endTime time in milliseconds representing the end of the interval
	 * @return the milliseconds within one or more Sundays
	 */
	public static int getMillisWithinSunday(final long startTime, final long endTime)
	{
		...
		if (currentWeekday == Calendar.SUNDAY) {
			if (involvedDays > 1) {
				...
				millisInSunday = millisInSunday - (IConstants.DAY - getMillisInDay(endTime));
			}
		}
	 ...
	 */
	public static int getMillisWithinWeekend(final long startTime, final long endTime)
	{
		if (startTime >= endTime) {
			return 0;
		}
		...
		if (currentWeekday <= Calendar.SUNDAY) {
			if (involvedDays > 1) {
				...
				millisInWeekend = millisInWeekend - (IConstants.DAY - getMillisInDay(endTime));
			}
		}
	...
	/**
	 * Calculates the drive forbid time (german "Sonntagsfahrverbot") interval in
	 * milliseconds.
	 * 
	 * This interval is from Sunday 0:00a.m. till Sunday 10:00p.m.
	 * 
	 * @param lastDeparture start time of a leg
	 * @param arrivalTime end time of a leg
	 * @return the drive forbid time (german "Sonntagsfahrverbot") interval in
	 *         milliseconds
	 */
	public static long getDriveRestrictionTimeMillis(final long lastDeparture, final long arrivalTime)
	{
		if (lastDeparture >= arrivalTime) {
			return 0;
		}

		int involvedDays = LongDateCalculator.getInvolvedDays(lastDeparture, arrivalTime);
		...
		if (currentWeekday == Calendar.SUNDAY) {
			...
			if (millisInDay < (22 * IConstants.HOUR)) {
				// the start is on Sunday, it must be waited till 10:00p.m. to start
				// driving
				millisDriveForbid = IConstants.DAY
						- LongDateCalculator.getMillisInDay(lastDeparture) - (2 * IConstants.HOUR);
			}
		}
	 ...
	 * 
	 * 
	 * @param timeMillisBase to calculate the nextSunday
	...
	/**
	 * Returns the next Saturday (0:00) in milliseconds, except the given date is
	 * within Saturday, then the given date (which is Saturday) is returned.
	 * 
	 * 
	 * @param timeMillisBase to calculate the nextSaturday
	 * @return the next Saturday (0:00) in milliseconds, except the given date is
	 *         within Saturday, then the given date (which is Saturday) is
	 *         returned.
	 */
	 ...
	 * 
	 * 
	 * @param timeMillisBase to calculate the previousSunday
